...
class JoiningSetAttribute<E extends IEntity, R> implements SetAttribute<E,R>, JoiningAttribute {
    
    private final List<? extends Attribute<?, ?>> attributes;

    <Y extends IEntity, A extends Attribute<?,?> & Bindable<?>> JoiningSetAttribute(Iterable<A> attrs) {
    ...
    public Class<Set<R>> getJavaType() {
        return (Class<Set<R>>) last(attributes).getJavaType();
    }
